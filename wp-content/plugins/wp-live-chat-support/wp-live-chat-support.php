...
function wplc_set_defaults() {
  ...
  if (empty($wplc_default_settings_array)) {
    $wplc_default_settings_array = array(
      ...
      'wplc_redirect_to_thank_you_page' => '',
...
add_action('init', 'wplc_start_session', 1);
...
function wplc_hook_control_check_mobile() {
  ...
  if ($wplc_settings['wplc_hide_when_offline'] && !apply_filters("wplc_loggedin_filter", false)) { // if wplc_hide_when_offline and no agents online, hide
    $draw_box = false;
  }
...
function wplc_add_user_stylesheet() {
  ...
  if ($show_chat_contents) {
    if ($wplc_settings['wplc_enable_font_awesome']) {
      wp_register_style( 'wplc-font-awesome', plugins_url( '/css/fontawesome-all.min.css', __FILE__ ) );
...
function wplc_add_admin_stylesheet() {
  ...
  wp_register_style( 'wplc-font-awesome', plugins_url('css/fontawesome-all.min.css', __FILE__ ), false, WPLC_PLUGIN_VERSION );
  wp_enqueue_style( 'wplc-font-awesome' );
  ...
  if (isset($_GET['page']) && $_GET['page'] == "wplivechat-menu-support-page") {
    wp_register_style('fontawesome', plugins_url('css/fontawesome-all.min.css', __FILE__ ), false, WPLC_PLUGIN_VERSION );
...
<?php
    ...
    $wplc_data['wplc_environment'] = wplc_force_int($_POST, 'wplc_environment', $wplc_settings);
    if ($wplc_data['wplc_environment']<0 || $wplc_data['wplc_environment']>4) {
      $wplc_data['wplc_environment']=2;
    }
    ...
    $wplc_data['wplc_settings_enabled'] = wplc_force_int($_POST, 'wplc_settings_enabled', $wplc_settings);
    ...
    $wplc_data['wplc_auto_pop_up'] = wplc_force_int($_POST, 'wplc_auto_pop_up');
    if ($wplc_data['wplc_auto_pop_up']<0 || $wplc_data['wplc_auto_pop_up']>2) {
      $wplc_data['wplc_auto_pop_up']=0;
    }
    ...
    $wplc_data['wplc_new_chat_ringer_count'] = wplc_force_int($_POST, 'wplc_new_chat_ringer_count', $wplc_settings);
    ...
    $wplc_data['wplc_elem_trigger_action'] = wplc_force_int($_POST, 'wplc_elem_trigger_action', $wplc_settings);
    $wplc_data['wplc_elem_trigger_type'] = wplc_force_int($_POST, 'wplc_elem_trigger_type', $wplc_settings);
    ...
    $wplc_data['wplc_gdpr_notice_retention_period'] = wplc_force_int($_POST, 'wplc_gdpr_notice_retention_period', $wplc_settings);
    if ($wplc_data['wplc_gdpr_notice_retention_period']<1) {
      $wplc_data['wplc_gdpr_notice_retention_period']=1;
    } else if ($wplc_data['wplc_gdpr_notice_retention_period']>730) {
      $wplc_data['wplc_gdpr_notice_retention_period']=730;
    }
    ...
    $wplc_data['wplc_bh_interval'] = wplc_force_int($_POST, 'wplc_bh_interval');
    if ($wplc_data['wplc_bh_interval']<0 || $wplc_data['wplc_bh_interval']>2) {
      $wplc_data['wplc_bh_interval']=0;
    }

    $wplc_data['wplc_bh_hours_start'] = wplc_force_int($_POST, 'wplc_bh_hours_start');
    if ($wplc_data['wplc_bh_hours_start']<0 || $wplc_data['wplc_bh_hours_start']>23) {
      $wplc_data['wplc_bh_hours_start']=0;
    }

    $wplc_data['wplc_bh_minutes_start'] = wplc_force_int($_POST, 'wplc_bh_minutes_start');
    if ($wplc_data['wplc_bh_minutes_start']<0 || $wplc_data['wplc_bh_minutes_start']>59) {
      $wplc_data['wplc_bh_minutes_start']=0;
    }

    $wplc_data['wplc_bh_hours_end'] = wplc_force_int($_POST, 'wplc_bh_hours_end');
    if ($wplc_data['wplc_bh_hours_end']<0 || $wplc_data['wplc_bh_hours_end']>23) {
      $wplc_data['wplc_bh_hours_end']=0;
    }

    $wplc_data['wplc_bh_minutes_end'] = wplc_force_int($_POST, 'wplc_bh_minutes_end');
    if ($wplc_data['wplc_bh_minutes_end']<0 || $wplc_data['wplc_bh_minutes_end']>59) {
      $wplc_data['wplc_bh_minutes_end']=0;
    }    

    $wplc_data['wplc_iterations'] = wplc_force_int($_POST, 'wplc_iterations');
    if ($wplc_data['wplc_iterations'] < 10) { $wplc_data['wplc_iterations'] = 10; }
    if ($wplc_data['wplc_iterations'] > 200) { $wplc_data['wplc_iterations'] = 200; }

    $wplc_data['wplc_delay_between_loops'] = wplc_force_int($_POST, 'wplc_delay_between_loops')*1000;
    if ($wplc_data['wplc_delay_between_loops'] < 250000) { $wplc_data['wplc_delay_between_loops'] = 250000; }
